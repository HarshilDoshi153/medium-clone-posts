    ...
    <section className='size flex gap-[4rem] relative'>
      <div className='mt-[9rem] flex-[2]'>
        <div className='flex items-center gap-4'>
          <h2 className='text-3xl sm:text-5xl font-bold capitalize'>Harshil Doshi</h2>
        ...
        <div className='flex items-center gap-5 mt-[3rem] border-b border-gray-300 mb-[3rem]'>
          {
            activities.map((item) => (
              <div key={item.title} className={`py-[0.5rem] px-[1rem] ${item.title === currentActive.title ? "border-b border-gray-700" : ""}`}>
                <button className='' onClick={() => setCurrentActive(item)}>{item.title}</button>
              </div>
            ))
          }
        </div>
      ...
      <Modal modal={modal} setModal={setModal}>
        ...
        ${modal ? "translate-x-0" : "translate-x-[100%] md:translate-x-0"} transition-all duration-500`}>
          ...
          <div className='sticky top-7 flex flex-col justify-between'>
            <img className='w-[3.5rem] h-[3.5rem] object-cover rounded-full' src='/assets/profile.jpg' alt='profile' />
            <h2 className='py-2 font-bold capitalize'>Harshil Doshi</h2>
            <p className='text-gray-500 first-letter:uppercase text-sm'>I am a web developer in India</p>
